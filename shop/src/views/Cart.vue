  ...
  <div>
    ...
    <div class="card">
         <van-card v-for="(item,index) in productList" :key="index" :price="item.price" :desc="item.company"  :title="item.name" :thumb="item.img">
           <div slot="footer">
             <van-butter size="mini" @click="delCart(item._id,index)">删除</van-butter>
           </div>
         </van-card>
    </div>
...
export default {
  ...
  computed:{
    ...mapState(['userInfo']),
    totalPrice(){
      return this.productList.reduct((sum,elem)=>{
        sum+=elem.price;
        return sum;
      },0)*100;
    }
  ...
  methods:{
    ...
    delCart(id,index){
      //删除数据库中的数据，如果删除成功，进入回调函数，在回调函数中，如下代码(完成删除数据库)：
